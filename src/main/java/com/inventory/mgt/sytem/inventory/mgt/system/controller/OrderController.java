...
public class OrderController {
    ...
    public void salesOrder(@PathVariable("id") Long id, SalesOrder salesOrder) {
         ...
         // equate the id passed to the sales order id
//         List<SalesTransaction> collect =
//                 all.stream().filter(f -> f.getSalesOrder().getId() == id).collect(Collectors.toList());
//         System.out.println(collect);
//
//         // GET LIST OF QUANTITY SOLD
//         List<Long> quantity = collect.stream().map(m
//                 -> m.getQuantitySold()).collect(Collectors.toList());
//         //convert to list of strings
//         List<String> transform = Lists.transform(quantity, Functions.toStringFunction());
//
//         //Join the string
...
//
//
//         //2)GET LIST OF AMOUNT
//         List<Long> amount= collect.stream().map(m -> m.getTotalAmount()).collect(Collectors.toList());
//
//         List<String> transform1 = Lists.transform(amount, Functions.toStringFunction());
//         String joinAmount = String.join("\n", transform1);
//         System.out.println(amount);
//
//
//
//         //3)GET LIST OF PRODUCT NAME
//         List<String>brand_name =
//                 collect.stream().map(m -> m.getProduct().getBrandName()).collect(Collectors.toList());
//         System.out.println(brand_name);
//         String joinName = String.join("\n", brand_name);
//
//
//         //40 GET THE ADMIN WHO SOLD THE PRODUCT
//         String firstName = collect.stream().map(m -> m.getSalesOrder().getUser().getFirstName()).findFirst().get();
//         String lastName = collect.stream().map(m -> m.getSalesOrder().getUser().getLastName()).findFirst().get();
//
//         // 5)GET THE LIST OF PRODUCT PRICES
//         List<Long> product_price= collect.stream()
//                 .map(m -> m.getProduct().getProductPrice()).collect(Collectors.toList());
